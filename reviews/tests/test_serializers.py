...
def test_serializer_ok(user_with_profile):
    ...
    # оконченная подтверждённая бронь
...
def test_serializer_rejects_landlord_himself(user_with_profile):
    ll = user_with_profile(username="ll", role="landlord")
    listing = baker.make("listings.Listing", landlord=ll, status="available")
    b = baker.make("bookings.Booking", listing=listing, tenant=ll,  # владелец сам себе «арендатор»
                   start_date=date.today() - timedelta(days=5),
                   end_date=date.today() - timedelta(days=2),
                   status="confirmed")
...
def test_serializer_rejects_foreign_booking(user_with_profile):
    ...
    listing = baker.make("listings.Listing", landlord=ll, status="available")
    b = baker.make("bookings.Booking", listing=listing, tenant=t1,
                   start_date=date.today() - timedelta(days=5),
                   end_date=date.today() - timedelta(days=2),
                   status="confirmed")
...
def test_serializer_requires_confirmed_and_finished(user_with_profile):
    ...
    # pending — нельзя
    b_pending = baker.make("bookings.Booking", listing=listing, tenant=tt,
                           start_date=date.today() - timedelta(days=5),
                           end_date=date.today() - timedelta(days=2),
                           status="pending")
    ...
    # confirmed, но ещё не завершилась — нельзя
    b_future = baker.make("bookings.Booking", listing=listing, tenant=tt,
